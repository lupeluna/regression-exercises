...
def telco_split_continuous(df):
    '''
    This funtion takes in the telco_churn_data with continuous variables acquired by the get_telco_churn,
    performs a split and stratifies total_charges column.
    Return train, validate, and test dfs
    '''

    # 20% test, 80% train_validate
    train_validate, test = train_test_split(df, test_size=0.2,
                                           random_state=1349)
    # 80% train, validate: 30% validate, 70% train
...
def mall_customer_data():
    ...
    # reads SQL query into a DataFrame
    df = pd.read_sql(customers, get_connection('mall_customers'))
    return df
